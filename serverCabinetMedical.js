...
function init(port, applicationServerIP, applicationServerPort) {
    ...
    app.post('/rmPatient'
        ...
        var allPatients = doc.getElementsByTagName("patient"); //on récupère la liste des patients présents dans le XML
        var del = false;
        var i = 0;
        
		console.log(allPatients);
        
        while(i < allPatients.length && !del){
            var numeroPatientX = allPatients[i].getElementsByTagName("numero")[0].textContent;   
            var prenomPatientX = allPatients[i].getElementsByTagName("prenom")[0].textContent; 
            var nomPatientX = allPatients[i].getElementsByTagName("nom")[0].textContent; 
			
            if (req.body.numero === numeroPatientX || (req.body.prenom === prenomPatientX && req.body.nom === nomPatientX)) {
                doc.getElementsByTagName("patients")[0].removeChild(allPatients[i]);
            	del=true;
            }
        ...
        if (del === false) {
            res.writeHead(400);
            res.write("MH Gros problème... Patient pas trouvé ");
            res.end();
            return;
        }
        saveXML(doc, res);
